...
import { createVehicleSchema, CreateVehicleFormData } from '../../utils/validationSchemas';
import { vehicleApi } from '../../api/vehicleApi';
import { OwnerVehiclesScreenProps } from '../../types/navigation.types';
import { VEHICLE_TYPES } from '../../utils/constants';
import { VehicleType } from '../../types/vehicle.types';

const AddVehicleScreen: React.FC<OwnerVehiclesScreenProps<'AddVehicle'>> = ({ navigation }) => {
    const [step, setStep] = useState(1);
    const queryClient = useQueryClient();
    const { control, handleSubmit, formState: { errors }, setValue, watch } = useForm<CreateVehicleFormData>({
    ...
    const mutation = useMutation({
        mutationFn: (data: CreateVehicleFormData) => vehicleApi.addVehicle(data),
        onSuccess: () => {
            Toast.show({ type: 'success', text1: 'Vehicle Added! 🚗', text2: 'Under review by admin.' });
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
                ...
                {step === 1 && (
                    <View>
                        ...
                        <Controller control={control} name="name" render={({ field: { onChange, value } }) => (
                            <AppInput label="Vehicle Name *" value={value || ''} onChangeText={onChange} error={errors.name?.message} leftIcon="car" />
                        )} />

                        <Text style={styles.label}>Vehicle Type *</Text>
                        <View style={styles.typeGrid}>
                            {VEHICLE_TYPES.map((t) => (
                                <TouchableOpacity key={t} style={[styles.typeChip, selectedType === t && styles.typeChipActive]}
                                    onPress={() => setValue('type', t as VehicleType)}>
                        ...
                        <Controller control={control} name="brand" render={({ field: { onChange, value } }) => (
                            <AppInput label="Brand" value={value || ''} onChangeText={onChange} leftIcon="tag" />
                        )} />
                        <Controller control={control} name="model" render={({ field: { onChange, value } }) => (
                            <AppInput label="Model" value={value || ''} onChangeText={onChange} leftIcon="tag-text" />
                        )} />
                        <Controller control={control} name="license_plate" render={({ field: { onChange, value } }) => (
                            <AppInput label="License Plate" value={value || ''} onChangeText={onChange} leftIcon="card-text" autoCapitalize="characters" />
                        )} />
                ...
                {step === 2 && (
                    <View>
                        ...
                        <Controller control={control} name="per_hour_cost" render={({ field: { onChange, value } }) => (
                            <AppInput label="Per Hour Cost (₹) *" value={value ? String(value) : ''} onChangeText={(t) => onChange(parseFloat(t) || 0)} error={errors.per_hour_cost?.message} leftIcon="cash" keyboardType="decimal-pad" />
                        )} />
                        <Controller control={control} name="per_day_cost" render={({ field: { onChange, value } }) => (
                            <AppInput label="Per Day Cost (₹) *" value={value ? String(value) : ''} onChangeText={(t) => onChange(parseFloat(t) || 0)} error={errors.per_day_cost?.message} leftIcon="cash-multiple" keyboardType="decimal-pad" />
                        )} />
                        <Controller control={control} name="location_name" render={({ field: { onChange, value } }) => (
                            <AppInput label="Location Name" value={value || ''} onChangeText={onChange} leftIcon="map-marker" placeholder="e.g. Banjara Hills, Hyderabad" />
                        )} />
