...
const MyVehiclesScreen: React.FC<OwnerVehiclesScreenProps<'MyVehicles'>> = ({ navigation }) => {
    ...
    const renderItem = useCallback(({ item }: { item: Vehicle }) => (
        <VehicleCard vehicle={item} onPress={() => navigation.navigate('EditVehicle', { vehicleId: item.id })} variant="owner" />
    ), [navigation]);
    ...
    return (
        <View style={styles.container}>
            ...
            {isLoading ? <AppLoader /> : filtered.length === 0 ? (
                <AppEmpty icon="car-off" title="No vehicles" subtitle="Add your first vehicle to start earning!"
