...
const BookingFormScreen: React.FC<UserHomeScreenProps<'BookingForm'>> = ({ navigation, route }) => {
    const { vehicleId } = route.params;
    const [pickupDate, setPickupDate] = useState(new Date(Date.now() + 3600000));
    const [dropDate, setDropDate] = useState(new Date(Date.now() + 3600000 * 4));
    ...
    const createBookingMutation = useMutation({
        ...
        onSuccess: () => {
            Toast.show({ type: 'success', text1: 'Booking Requested! 🎉', text2: 'Waiting for owner approval' });
    ...
    return (
        <View style={styles.container}>
            ...
            <DatePicker modal open={showPickupPicker} date={pickupDate} onConfirm={(d) => { setPickupDate(d); setShowPickupPicker(false); }} onCancel={() => setShowPickupPicker(false)} minimumDate={new Date()} />
            <DatePicker modal open={showDropPicker} date={dropDate} onConfirm={(d) => { setDropDate(d); setShowDropPicker(false); }} onCancel={() => setShowDropPicker(false)} minimumDate={pickupDate} />
        </View>
    );
};
