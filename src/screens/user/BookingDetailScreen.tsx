...
import { UserBookingScreenProps } from '../../types/navigation.types';
...
const BookingDetailScreen: React.FC<UserBookingScreenProps<'BookingDetail'>> = ({ navigation, route }) => {
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
                ...
                {/* Status Stepper */}
                {currentStepIndex >= 0 && (
                    <View style={[styles.stepperCard, Shadows.card]}>
                ...
                {booking.status === 'ACCEPTED' && booking.owner && (
                    <View style={[styles.card, Shadows.card, { borderLeftWidth: 3, borderLeftColor: Colors.success }]}>
                ...
                {booking.status === 'PENDING' && (
                    <AppButton label="Cancel Booking" onPress={() => cancelMutation.mutate()} variant="danger" icon="close-circle" loading={cancelMutation.isPending} style={{ marginTop: Spacing.md }} />
                )}

                {booking.status === 'COMPLETED' && !booking.review && (
                    <AppButton label="Rate Your Experience" onPress={() => navigation.navigate('Review', { bookingId: booking.id, vehicleId: booking.vehicle_id })} variant="secondary" icon="star" style={{ marginTop: Spacing.md }} />
                )}
            </ScrollView>
        </View>
    );
