...
const HomeScreen = ({ navigation }) => {
  ...
  const { data, isLoading, refetch, isRefetching } = useQuery({
    ...
    queryFn: () => vehicleApi.searchVehicles({
      lat,
      lng,
      radius_km: SEARCH_RADIUS_DEFAULT,
  ...
  const renderVehicle = useCallback(({ item }) => <View style={styles.cardWrapper}>
  ...
  >
    ...
    /* Vehicle Type Filter */
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
                {vehicleFilters.map((filter) => <TouchableOpacity
    key={filter.value}
  ...
  >
    ...
    /* Vehicle List */
            ...
            {isLoading ? <AppLoader message="Finding vehicles..." /> : vehicles.length === 0 ? <AppEmpty icon="car-off" title="No vehicles found" subtitle="Try changing filters or expanding your search radius" /> : <FlatList
    data={vehicles}
