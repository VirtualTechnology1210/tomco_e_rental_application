...
const BookingHistoryScreen: React.FC<UserBookingScreenProps<'BookingHistory'>> = ({ navigation }) => {
    ...
    const cancelMutation = useMutation({
        mutationFn: (bookingId: string) => bookingApi.cancelBooking(bookingId, 'Cancelled by user'),
        onSuccess: () => {
    ...
    const renderBooking = useCallback(({ item }: { item: Booking }) => (
        <BookingCard booking={item} onPress={() => navigation.navigate('BookingDetail', { bookingId: item.id })} role="user" />
    ), [navigation]);
    ...
    return (
        <View style={styles.container}>
            ...
            {isLoading ? (
                ...
                <AppEmpty icon="calendar-blank" title="No bookings yet" subtitle={activeTab !== 'ALL' ? `No ${activeTab.toLowerCase()} bookings` : 'Start by booking a vehicle!'} />
            ) : (
                <FlatList
                    data={filtered}
