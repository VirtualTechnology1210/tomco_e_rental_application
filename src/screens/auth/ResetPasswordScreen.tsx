...
const ResetPasswordScreen: React.FC<AuthScreenProps<'ResetPassword'>> = ({ navigation, route }) => {
    ...
    const mutation = useMutation({
        mutationFn: (data: { new_password: string }) =>
            authApi.resetPassword({ phone, otp: userId, new_password: data.new_password }),
        onSuccess: () => {
            ...
            <View style={styles.content}>
                ...
                <Controller control={control} name="new_password"
                    render={({ field: { onChange, onBlur, value } }) => (
                        <AppInput label="New Password" value={value} onChangeText={onChange} onBlur={onBlur}
                            error={errors.new_password?.message} secureTextEntry leftIcon="lock" placeholder="Min 6 characters" />
                    )} />
