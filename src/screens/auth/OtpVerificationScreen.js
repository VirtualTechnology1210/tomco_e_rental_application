...
const OtpVerificationScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (timer <= 0) return;
    const interval = setInterval(() => setTimer((prev) => prev - 1), 1e3);
  ...
  />
                ...
                <View style={styles.resendRow}>
                    {timer > 0 ? <Text style={styles.timerText}>Resend OTP in {timer}s</Text> : <TouchableOpacity onPress={handleResend}>
...
var OtpVerificationScreen_default = OtpVerificationScreen;
export {
  OtpVerificationScreen_default as default
};
