...
import { signupSchema, ownerSignupSchema, SignupFormData, OwnerSignupFormData } from '../../utils/validationSchemas';
import { authApi } from '../../api/authApi';
import { COUNTRY_CODE } from '../../utils/constants';

const SignupScreen: React.FC<AuthScreenProps<'Signup'>> = ({ navigation, route }) => {
    const { role } = route.params;
    const isOwner = role === 'VEHICLE_OWNER';
    const schema = isOwner ? ownerSignupSchema : signupSchema;

    const { control, handleSubmit, formState: { errors } } = useForm<SignupFormData | OwnerSignupFormData>({
        resolver: yupResolver(schema),
        defaultValues: { full_name: '', phone: '', email: '', password: '', confirm_password: '' },
    });

    const signupMutation = useMutation({
        mutationFn: (data: SignupFormData | OwnerSignupFormData) =>
            authApi.signup({ full_name: data.full_name, phone: `${COUNTRY_CODE}${data.phone}`, email: data.email || undefined, password: data.password, role }),
        onSuccess: (response) => {
    ...
    const onSubmit = (data: SignupFormData | OwnerSignupFormData) => signupMutation.mutate(data);
                ...
                <View style={styles.form}>
                    <Controller control={control} name="full_name"
                        render={({ field: { onChange, onBlur, value } }) => (
                            <AppInput label="Full Name" value={value} onChangeText={onChange} onBlur={onBlur}
                                error={errors.full_name?.message} leftIcon="account" placeholder="Enter your full name" />
                        )} />

                    <Controller control={control} name="phone"
                        render={({ field: { onChange, onBlur, value } }) => (
                            <AppInput label="Phone Number" value={value} onChangeText={onChange} onBlur={onBlur}
                                error={errors.phone?.message} leftIcon="phone" placeholder="Phone number" keyboardType="phone-pad" />
                        )} />

                    <Controller control={control} name="email"
                        render={({ field: { onChange, onBlur, value } }) => (
                            <AppInput label="Email (Optional)" value={value || ''} onChangeText={onChange} onBlur={onBlur}
                                error={errors.email?.message} leftIcon="email" placeholder="Email address" keyboardType="email-address" autoCapitalize="none" />
                        )} />

                    <Controller control={control} name="password"
                        render={({ field: { onChange, onBlur, value } }) => (
                            <AppInput label="Password" value={value} onChangeText={onChange} onBlur={onBlur}
                                error={errors.password?.message} secureTextEntry leftIcon="lock" placeholder="Min 6 characters" hint="Minimum 6 characters" />
                        )} />

                    <Controller control={control} name="confirm_password"
                        render={({ field: { onChange, onBlur, value } }) => (
                            <AppInput label="Confirm Password" value={value} onChangeText={onChange} onBlur={onBlur}
                                error={errors.confirm_password?.message} secureTextEntry leftIcon="lock-check" placeholder="Re-enter password" />
                        )} />
                    ...
                    {isOwner && (
                        <>
                            ...
                            <Controller control={control} name="business_name" defaultValue=""
                                render={({ field: { onChange, onBlur, value } }) => (
                                    <AppInput label="Business Name" value={value || ''} onChangeText={onChange} onBlur={onBlur}
                                        leftIcon="store" placeholder="Your business name" />
                                )} />
                            <Controller control={control} name="city" defaultValue=""
                                render={({ field: { onChange, onBlur, value } }) => (
                                    <AppInput label="City" value={value || ''} onChangeText={onChange} onBlur={onBlur}
                                        leftIcon="city" placeholder="Your city" />
                                )} />
                        </>
                    )}
