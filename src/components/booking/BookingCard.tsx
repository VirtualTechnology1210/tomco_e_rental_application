...
const BookingCard: React.FC<BookingCardProps> = ({ booking, onPress, role = 'user', onAccept, onReject }) => {
        ...
        <TouchableOpacity style={[styles.card, Shadows.card]} onPress={onPress} activeOpacity={0.85}>
            <View style={styles.row}>
                <View style={styles.imageBox}>
                    {vehicleImage ? (
                        <Image source={{ uri: vehicleImage }} style={styles.image} resizeMode="cover" />
                    ) : (
                ...
                <View style={styles.info}>
                    ...
                    {role === 'owner' && booking.user && (
                        <Text style={styles.userName}>👤 {booking.user.full_name}</Text>
                    )}
            ...
            {role === 'owner' && booking.status === 'PENDING' && onAccept && onReject && (
