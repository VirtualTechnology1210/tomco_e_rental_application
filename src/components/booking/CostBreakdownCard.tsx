...
const CostBreakdownCard: React.FC<CostBreakdownCardProps> = ({
    perHourCost, perDayCost, totalHours, totalDays, totalCost,
    billingMode, commissionPercent = 10,
}) => {
    ...
    return (
        <View style={[styles.card, Shadows.card]}>
            ...
            <View style={styles.row}>
                ...
                <Text style={styles.value}>{billingMode === 'HOURLY' ? '⏰ Hourly' : '📅 Daily'}</Text>
            </View>
            ...
            <View style={styles.row}>
                ...
                <Text style={styles.value}>
                    {billingMode === 'HOURLY'
                        ? `${Math.ceil(totalHours)} hour${Math.ceil(totalHours) !== 1 ? 's' : ''}`
                        : `${Math.ceil(totalDays)} day${Math.ceil(totalDays) !== 1 ? 's' : ''}`}
                </Text>
            </View>
